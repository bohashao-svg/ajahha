...
struct OutputCard: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text(item.taskName ?? "未命名")
                    ...
                    .foregroundColor(.rhPrimary).lineLimit(1)
                ...
                HStack(spacing: 4) {
                    ...
                    Text(statusText(item.taskStatus))
                        .font(.system(size: 11)).foregroundColor(statusColor(item.taskStatus))
                }
            }
            Spacer()
        ...
        .confirmationDialog("选择操作", isPresented: $showActionSheet, titleVisibility: .visible) {
            if item.outputType?.lowercased() != "zip" {
                Button("保存原图到相册") { Task { await saveImage(useOriginal: true) } }
                Button("保存预览图到相册") { Task { await saveImage(useOriginal: false) } }
            }
    ...
    private func saveImage(useOriginal: Bool) async {
        let urlStr = useOriginal ? (item.fileUrl ?? "") : (item.filePreviewUrl ?? "")
        guard let url = URL(string: urlStr) else { showToast("链接无效"); return }
        isSaving = true
        do {
            let (data, _) = try await URLSession.shared.data(from: url)
            guard let image = UIImage(data: data) else {
                showToast("图片解析失败"); isSaving = false; return
            }
    ...
    private func saveUIImageToAlbum(_ image: UIImage) async {
        ...
        if status == .notDetermined {
            ...
            if !granted { showToast("未授权访问相册"); return }
        }
        do {
        ...
        NavigationView {
            VStack(spacing: 24) {
                ...
                if let err = errorMessage {
                    HStack(spacing: 6) {
                        Image(systemName: "exclamationmark.circle.fill").foregroundColor(.rhError)
                        Text(err).font(.system(size: 13)).foregroundColor(.rhError)
                    }
                    .padding(10)
                ...
                if let err = vm.errorMessage {
                    VStack(spacing: 12) {
                        Image(systemName: "exclamationmark.circle")
                            .font(.system(size: 36)).foregroundColor(.rhBorder)
                        Text(err).font(.system(size: 14)).foregroundColor(.rhSecondary)
                            .multilineTextAlignment(.center).padding(.horizontal, 32)
                        Button("重试") { Task { await vm.loadPage(1) } }
                            .font(.system(size: 14, weight: .medium)).foregroundColor(.rhAccent)
                    }
                } else {
                    outputList
                }
